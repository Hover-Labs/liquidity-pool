...
class Contract(sp.Contract):
  ...
  def getAdministrator(self, params):
    ...
    __s233 = sp.local("__s233", self.data.administrator)
    sp.set_type(sp.snd(params), sp.TContract(sp.TAddress))
    sp.transfer(__s233.value, sp.tez(0), sp.snd(params))

  @sp.entry_point
  def getAllowance(self, params):
    __s234 = sp.local("__s234", self.data.balances[sp.fst(params).owner].approvals[sp.fst(params).spender])
    sp.set_type(sp.snd(params), sp.TContract(sp.TNat))
    sp.transfer(__s234.value, sp.tez(0), sp.snd(params))

  @sp.entry_point
  def getBalance(self, params):
    sp.if ~ (self.data.balances.contains(sp.fst(params))):
      self.data.balances[sp.fst(params)] = sp.record(approvals = {}, balance = 0)
    __s235 = sp.local("__s235", self.data.balances[sp.fst(params)].balance)
    sp.set_type(sp.snd(params), sp.TContract(sp.TNat))
    sp.transfer(__s235.value, sp.tez(0), sp.snd(params))

  @sp.entry_point
  def getTotalSupply(self, params):
    sp.set_type(sp.fst(params), sp.TUnit)
    __s236 = sp.local("__s236", self.data.totalSupply)
    sp.set_type(sp.snd(params), sp.TContract(sp.TNat))
    sp.transfer(__s236.value, sp.tez(0), sp.snd(params))
